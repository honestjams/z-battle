...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Check for any existing pending challenges
  ...
  const handleIntent = useCallback((intent: Intent) => {
    ...
    if (newState.turnPlayer !== prevTurnPlayer) {
      if (aiPlayer) {
        setScreen('game'); // AI games never pass the device
      } else {
        setScreen('pass'); // Hotseat: show the overlay before the next player sees their hand
      }
    }
  }, [gameState, aiPlayer]);
  ...
  }, [aiPlayer, screen, gameState, handleIntent, pendingAiAttack]);
  ...
  async function handleAcceptChallenge() {
    ...
    const gameState = createInitialState(m.player1_deck!, acceptDeck, firstPlayer);

    await supabase.from('matches').update({
      player2_deck: acceptDeck,
      state: gameState,
      status: 'active',
      turn_player: gameState.turnPlayer === 'p1' ? m.player1 : user.id,
